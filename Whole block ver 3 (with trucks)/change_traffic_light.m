...
%pressure of each approach
    left_press = sum(hor_lane(1,intersection(2)-8:intersection(2)-1)>=0 & hor_lane(1,intersection(2)-8:intersection(2)-1)<6); 
    top_press = sum(vert_lane(1,intersection(1)-8:intersection(1)-1)>=0 & vert_lane(1,intersection(1)-8:intersection(1)-1)<6);
    if hor_lane(1,intersection(2)) == 6, left_red = 1;top_red = 0;else, left_red = 0;top_red = 1; end
    
    if(delay_signal~=0 || count_red_top==1 || count_red_left==1)
        %don't change the traffic lights cause a car is on top of the inter
        %the other 2 for min green lights
    %max_red   
