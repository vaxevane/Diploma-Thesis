function[lane,park_signals,park_flags,change_signals] = move(lane,new_car_prob,park_signals,park_flags,dec_prob,park_prob)
    [d,c] = size(lane);
    d=1;
        ...
        for i=1:d          
            ...
            if(rand_dec>=0 && rand_dec<6 && (park_signals(i,j)<1 && park_signals(i,j)>-1))%<1 bec if he already want to park he should stay commited in order to park
                if(park_signals(i,j)<0)
                    park_signals(i,j) = park_signals(i,j)-0.25;
                else
                    park_signals(i,j) = park_signals(i,j)+0.25;
                end
            end
