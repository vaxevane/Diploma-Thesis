...
for e=4:4:iter
        ...
        for i=1:d          
            ...
            if(rand_dec>=0 && park_signals(i,j)<1)%<1 bec if he already want to park he should stay commited in order to park
                if(rand_dec>3)
                    ...
                    park_signals(i,j) = park_signals(i,j)+0.4;
                end
            end
    ...
    for s=1:length(park_temp)
        ...
        if(1)
            ...
            mask = find(park_flags~=0);
            mask = mask(find(mask==park_temp(s)):end);
            %taking the flags of the car and after
            moving_flags = exclude_already_flagged(find(exclude_already_flagged==park_flags(1,park_temp(s))):end);
            park_flags(mask) = 0;
            lane(d+1,mask) = -1;
            %Showing the spot that is taken is unavailable
            lane(d+1,park_temp(s)) = 0;
            for re=1:length(mask)
                new_flag = find(lane(d+1,mask(re):end)==-1,1)+mask(re)-1;
                if(new_flag<=last_park)
                    park_flags(new_flag) = moving_flags(re);
                    lane(d+1,new_flag) = 7;
                    if(new_flag~=mask(re))
